...
function buildOpenApiPaths(config) {
  ...
  for (const tool of config.tools ?? []) {
    ...
    const operation = {
      ...
      "x-lumo-tool": true,
      "x-lumo-cost-tier": toolCostTier(config, tool),
      "x-lumo-requires-confirmation": toolConfirmation(config, tool),
      "x-lumo-pii-required": tool.piiRequired ?? [],
      "x-lumo-intent-tags": tool.intentTags ?? [config.agentId],
    };

    if (tool.cancels) operation["x-lumo-cancels"] = tool.cancels;
    if (tool.cancelFor) operation["x-lumo-cancel-for"] = tool.cancelFor;
    if (tool.compensationKind) operation["x-lumo-compensation-kind"] = tool.compensationKind;
    if (tool.reversibility) operation["x-lumo-reversibility"] = tool.reversibility;
    if (tool.compensatingTool) operation["x-lumo-compensating-tool"] = tool.compensatingTool;
    if (tool.compensatingInputsTemplate) {
      operation["x-lumo-compensating-inputs-template"] = tool.compensatingInputsTemplate;
    }
    if (typeof tool.compensatingWindowSeconds === "number") {
      operation["x-lumo-compensating-window-seconds"] = tool.compensatingWindowSeconds;
    }
